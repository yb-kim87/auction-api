...
## 追記 (2026-08-11) — 여전히 대부분 실패해 완전 순차 처리로 전환

버그 수정 배포 후 세종 전체를 다시 돌려보니, 조기 종료는 더 이상
안 하지만(수정 확인) 여전히 배치의 2/3가량이 계속 실패하고 있었다.
실패 로그 간격(배치당 약 30초)이 재시도 3회×약 10초와 일치해, WAF
차단이라기보다 **동시 요청이 2~3개만 겹쳐도 응답이 느려져 타임아웃
되는 것**으로 추정된다.

- `LIST_CONCURRENCY`를 1(완전 순차)로, `DETAIL_CONCURRENCY`는 3으로
  낮춤.
- Vercel 프록시(`proxy/route.ts`)에 `maxDuration=30`, 개별 fetch에
  12초 타임아웃(`AbortSignal.timeout`)을 추가 — 무한정 매달리다
  Vercel 함수 자체가 죽는 것보다, 짧게 실패시켜 백엔드 재시도가 더
  빨리 판단하게 한다.

### 검증
배포 후 세종 전체 재수집으로 재검증 예정. 순차 처리라 이전보다 더
오래 걸릴 수 있음(속도보다 완전한 수집을 우선).

## 追記 (2026-08-11) — 상세 수집도 완전 순차로, 중단 기능 추가

사용자가 "지금 테스트 하는거 너무 오래 걸리는거같은데"라고 지적 —
확인해보니 `DETAIL_CONCURRENCY=3`으로 남겨둔 상세 페이지 동시요청이
여전히 느렸을 뿐 아니라, 같은 시점에 호출되는 지역콤보
(ajax_combo_search.asp) 요청까지 실패시키고 있었다(수집 중에
관리자 페이지에서 시/군/구 드롭박스를 열면 "전체"만 보이던 원인 —
드롭박스 버그가 아니라 리소스 경합이었음, curl로 직접 재현 확인).

- `DETAIL_CONCURRENCY`도 1(완전 순차)로 낮춤 — 이제 목록/상세 모두
  동시 요청이 전혀 없다.
- `POST /realtor-collect/stop` 신규: 재배포 없이도 진행 중인 수집을
  즉시 중단할 수 있도록 취소 플래그(`cancelRequested`)를 추가하고,
  목록/상세 루프 시작 지점마다 확인해 중단한다. 프론트 "실행" 버튼
  옆에 진행 중일 때만 "중단" 버튼 노출.

### 검증
배포(=진행 중이던 정체 작업 자동 종료) 후, 작은 지역 기준 완전 순차
수집 + 중단 버튼 동작 재검증 예정.

## 追記 (2026-08-11) — 목록 건수 확인 후 진행 여부를 확인받는 2단계 흐름

사용자 요청: "주소들을 다 입력한 후에 리스트가 몇개인지 확인하고
작업을 진행하게 할 수 있을까?" — 지역을 잘못 넓게 선택해 수천 건
상세수집을 실수로 돌리는 걸 막기 위해, 실행을 목록 확인/상세 수집
2단계로 나눴다.

- `JobState`에 `phase`(idle/listing/awaiting_confirmation/collecting)
  추가.
- "실행"을 누르면 우선 목록 페이지만 순회해 건수를 세고(상세 페이지는
  아직 안 건드림) `awaiting_confirmation` 상태로 멈춘다 — 이미 모은
  행 목록(`pendingRows`)은 메모리에 캐싱해둔다.
- 화면에 "OO시 OO구 목록 확인 완료 — 총 N건. 상세정보를 수집할까요?"
  배너와 "상세정보 수집 시작"/"취소" 버튼을 노출.
- `POST /realtor-collect/confirm`(신규): 캐싱해둔 목록을 그대로 이어서
  상세 수집을 시작 — 목록을 다시 긁지 않는다.
- `stop()`을 "확인 대기 중 취소"와 "진행 중 중단" 두 경우 모두
  처리하도록 확장(확인 대기 중엔 비동기 루프가 없어 캔슬 플래그로는
  못 멈추므로 즉시 초기화).

### 검증
백엔드/프론트 각각 `npx tsc --noEmit` 통과. 배포 후 실제 흐름(목록
확인 → 배너 표시 → 확인/취소) 재검증 예정.

## 追記 (2026-08-11) — 지역 드롭박스가 여전히 비어 보임: 서비스 전역 요청 큐로 해결

동시성을 서비스 내부 루프에서 1로 낮췄는데도, 사용자가 실제로
목록 확인 진행 중(페이지 6~12 수집 중) 시/군/구 드롭박스를 열면
여전히 "전체"만 보인다고 재현 스크린샷으로 지적 — 사용자가 직접
"중단"을 누른 로그도 함께 확인됨.

### 원인
`LIST_CONCURRENCY`/`DETAIL_CONCURRENCY`는 **한 작업(job) 안의
루프 하나만** 순차로 만들 뿐, 그 루프와 완전히 별개인
`fetchSubOptions()`(드롭박스 클릭 시 호출)는 동시에 실행되는 걸
막지 못했다 — 서로 다른 코드 경로라 "동시성 설정"의 적용 범위 밖에
있었음.

### 수정
`RealtorCollectService`에 전역 요청 큐(`requestQueue`, 프라미스
체인 기반 뮤텍스)를 추가해 `proxyFetch()`(모든 karhanbang.com
호출의 유일한 출구) 자체를 감쌌다 — 이제 목록 수집이든 상세
수집이든 드롭박스 조회든, 이 서비스가 만드는 karhanbang.com 요청은
**출처와 무관하게 항상 전역으로 한 번에 하나씩만** 나간다(사이 150ms
대기). 개별 루프의 CONCURRENCY 상수는 더 이상 실제 동시 요청 수를
좌우하지 않지만(전역 큐가 어차피 직렬화), 코드 구조상 그대로 둬도
무해해 남겨뒀다.

### 검증
배포 후 목록 확인 진행 중에 드롭박스를 열어도 정상적으로 지역
목록이 뜨는지 재확인 예정.
