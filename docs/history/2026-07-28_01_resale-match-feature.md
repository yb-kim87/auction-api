...
### 최초 매칭 배치 실행 결과 (2026-07-28 10:30~10:37)
타임아웃 수정 배포 후 실거래 수집(58,941건 신규/갱신)과 스코어링이
정상 완료됨(더 이상 크래시/hang 없음). 완납일이 확보된 9건 중
실제로 스코어링까지 간 것은 2건(나머지는 같은 주소의 실거래
후보가 아직 없음 — 국토부 API가 해당 시군구·기간에 거래 자체가
없거나, 다음 배치 주기(1일 1회)에 더 최근 실거래가 추가로
잡혀야 하는 상황).

- **표시(isDisplayed=true)된 매칭은 0건.** 최고 등급(HIGH, 75점)
  후보가 나온 사례(서울 강남구 논현동 마일스디오빌)가 하나 있었지만,
  2위 후보와 점수차가 1점(75 vs 74)으로 애매성 임계값(8점 미만
  차이는 표시 안 함)에 걸려 **설계 원칙(오탐 방지 최우선)대로
  자동 억제**됐다. 나머지 후보들은 대부분 LOW~MEDIUM 등급(29~65점)
  으로 애초에 표시 기준(70점 이상)에 못 미침.
- 즉 이번 백필 9건 범위에서는 "확신을 갖고 사용자에게 보여줄 만한"
  매칭 사례는 아직 없음 — 이는 매칭 로직이 오작동한 게 아니라
  설계 의도(정확도 > 매칭률)가 정상적으로 작동한 결과. 표본이
  9건뿐이라 통계적으로도 아직 이르다.
- 매일 1회 자동 배치가 계속 도므로, 완납 물건이 늘고 국토부 실거래
  데이터가 축적되면 확신도 높은 매칭이 나올 가능성은 유지된다.
  현재 24시간 스케줄러가 정상 등록돼 있어 별도 조치 불필요.

## 追記 (2026-07-28) — 층을 스코어링 감점 요소에서 하드필터로 변경

### 배경
최초 매칭 배치 결과를 사용자와 함께 검토하던 중(2024타경110655,
논현동 마일스디오빌 사례), 3층 물건에 대해 1위 후보(3층, 75점)와
2위 후보(4층, 인접층 감점만 받고 74점)가 거의 동점이 되어 애매
판정으로 억제된 사례가 나왔다. 사용자 피드백: "이 작업은 실제로
낙찰된 물건이 실거래로 이어졌는지 보는 게 주요 관점이라, 층이
다르면 의미가 없다 — 인접층 감점 로직은 완전히 빼야 한다."

### 변경 내용
- `resale-match.service.ts`의 `matchOne()`: 경매 물건의 층
  (`parseAuctionFloor(auction.address)`)을 후보 조회 쿼리의
  **하드필터**로 추가(`t.floor = :floor`) — 층이 다른 실거래는
  애초에 후보 목록에 들어오지 않는다. 경매 물건 자체의 층을
  주소에서 특정할 수 없으면(예: 층 표기가 애매한 주소) 검증이
  불가능하므로 매칭을 아예 시도하지 않는다.
- `match-scoring.util.ts`의 `floorScore()`: 인접층 0.5점 부분점수
  로직 제거 — 이제 하드필터를 통과한 후보는 항상 층이 일치하므로
  단순 방어적 재확인만 남김(불일치 시 0, 일치 시 1).
- 고유성 보정(`candidateUnitCount`) 기준도 "단지 내 서로 다른 층 수"
  에서 "동일 호실 조건(주소+층+면적)에 잡히는 거래 건수"로 의미가
  바뀜에 따라 계산식을 `candidates.length` 기준으로 단순화.
