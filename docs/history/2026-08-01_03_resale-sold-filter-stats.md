...
## 追記 (2026-08-04) — 지도 마커 색상을 등급이 아닌 매도차익 금액 기준으로 변경

사용자 요청: "매도분석관련 지도에 찍히는 색상을 매도차익 금액으로
하면 좋을꺼같아 +금액이 클수록 좋은거고 -나온거는 안좋은거고 이런식으로
색상 기준점을 바꿔줘". 기존엔 `confidenceTier`(VERY_HIGH/HIGH/MEDIUM)
4단계 고정색이었는데, 매도차익(실거래가-낙찰가) 값 자체를 색으로
표현하도록 변경.

`src/lib/kakao-maps.ts`에 `profitToColor(profit)` 추가 — 0 근처는
중립 회색(#9ca3af), 이익 방향은 진한 초록(#065f46, +1억에서 포화),
손해 방향은 진한 빨강(#b91c1c, -5천만원에서 포화)으로 선형 보간하는
다이버징(diverging) 색상 스케일. 마커 이미지 캐시는 무한히 다양한
색을 다 만들지 않도록 500만원 단위로 반올림해 캐시 키를 잡음(연속값
색상표라 그대로 쓰면 캐시 히트가 거의 안 나서 성능이 나빠짐).
`ResaleMatchMapView.tsx`의 등급별 범례도 매도차익 구간별 범례로 교체.

### 변경 파일
`src/lib/kakao-maps.ts`, `src/app/admin/ResaleMatchMapView.tsx` (auction).

### 테스트 결과
`tsc --noEmit` + `npm run build` 클린. 이번엔 로컬 작업 디렉토리에
