...
## 향후 고려사항: `auction-courses.vercel.app` 직접 접속 허용 여부

2026-08-20, 배포 직후 사용자가 Vercel 대시보드에서 `auction-courses`
프로젝트 자체 도메인(`auction-courses.vercel.app`)으로 들어가보니 루트(`/`)가
404가 뜨는 걸 보고 "이 주소로도 바로 들어가지게 하고 싶다"는 질문이 나옴.
지금 당장 적용하지 않고 **향후 강의실을 로그인까지 완전히 별도 서비스로
분리할 때** 다시 요청하기로 함 — 아래 내용을 참고해서 그때 진행한다.

**지금 이 주소로 못 들어가는(정확히는 반쪽만 되는) 이유**
1. `basePath: "/courses"`라 루트(`/`) 자체가 404 — `/courses`를 붙이면 뜸.
2. courses 앱엔 자체 `/api/[...path]/route.ts` 프록시가 없음(outer 앱에만
   있음) — 그래서 이 도메인으로 직접 들어오면 데이터 fetch가 실패함.
3. courses 앱엔 자체 `/login` 페이지가 없음(outer 전용) — 비로그인 시
   미들웨어가 리다이렉트하려 해도 갈 곳이 없음.

**핵심 트레이드오프 — 로그인 세션 공유가 깨짐**
지금 outer/courses 두 앱이 같은 로그인으로 동작하는 이유는 브라우저가
"하나의 도메인(outer)"만 보고 있어서 쿠키가 자연히 공유되기 때문이다.
`auction-courses.vercel.app`을 별도 진입점으로 열어버리면 그 순간부터는
다른 origin이 되어 쿠키가 공유되지 않는다 — 즉 **"이 주소로도 들어가지게
하면서 동시에 지금처럼 로그인은 계속 공유되게"는 아키텍처적으로
불가능**하다. 이 주소를 정식으로 열려면 사실상 강의실을 로그인까지
독립된 별개 서비스로 취급하겠다는 결정이 선행되어야 한다.

**향후 실제로 진행하게 되면 필요한 작업**
1. `auction-courses`에 자체 `/api/[...path]/route.ts` 프록시 추가(outer의
   구현 참고).
2. `auction-courses`에 자체 로그인 페이지/플로우 구성(또는 outer 로그인
   페이지로의 리다이렉트 — 다만 이 경우도 세션은 안 넘어옴).
3. 루트(`/`) 처리 — 랜딩페이지 신설 또는 `/courses`로 리다이렉트.
4. (선택) 완전히 갈라선다면 커스텀 도메인 분리, 별도 브랜딩 등도 함께
   검토.

사용자가 "완전 분리(로그인도 별도)"를 결정하는 시점에 다시 요청하기로
합의함 — 그 전까지는 `auction-courses.vercel.app`은 outer 리라이트 경유
전용으로만 쓰고, 직접 진입은 의도적으로 반쪽짜리 상태로 남겨둔다.
