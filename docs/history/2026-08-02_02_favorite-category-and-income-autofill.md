...
## 追記 (2026-08-05) — 관심 하트 즉시 반응(낙관적 UI)

관심 하트를 누른 뒤 API 응답을 기다린 다음에야 채움 상태와 관심물건 수가
바뀌어 클릭 반응이 느리게 느껴졌다.

- 추천 물건, 전체 검색, 내 물건 화면에서 클릭 즉시 로컬 관심 상태를
  먼저 갱신하도록 변경했다.
- API 호출이 성공하면 즉시 반영된 상태를 그대로 유지한다.
- 호출 실패 시에는 해당 물건만 클릭 전 관심 상태·카테고리·메모로 복원해
  다른 카드에서 동시에 일어난 변경을 덮어쓰지 않도록 했다.
- 같은 물건의 요청 중복 방지용 비활성 상태는 유지한다.

### 변경 파일 및 검증
`auction/src/app/page.tsx`, `auction/src/app/search/page.tsx`,
`auction/src/app/favorites/page.tsx`. 프론트엔드 `npx tsc --noEmit` 및
로컬 프론트 응답 확인 통과.
