...
export function mapCrawledItem(raw: Record<string, unknown>): Partial<UpdateAuctionDto> {
  ...
  return {
    ...
    unpaidFeeAmount: num(raw.unpaidFeeAmount ?? raw.unpaid_fee_amount),
    unpaidFeeNote: str(raw.unpaidFeeNote ?? raw.unpaid_fee_note),
    unpaidFeeCheckedAt: str(raw.unpaidFeeCheckedAt ?? raw.unpaid_fee_checked_at),
    lawdCd: str(raw.lawdCd ?? raw.lawd_cd) || null,
    umdNm: str(raw.umdNm ?? raw.umd_nm) || null,
    jibun: str(raw.jibun) || null,
    saleConfirmedAt: str(raw.saleConfirmedAt ?? raw.sale_confirmed_at) || null,
    paymentCompletedAt: str(raw.paymentCompletedAt ?? raw.payment_completed_at) || null,
    elevator,
    parking,
